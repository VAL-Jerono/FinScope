...
def load_enhanced_data():
    ...
    # For mapping visualization, create country dataframe using regional averages (NO SYNTHETIC INDIVIDUAL RATES)
    # Each country gets its region's actual inclusion rate from your analysis
    country_coordinates = {
        # Just coordinates and population - inclusion rates come from regional data
        'Australia': {'lat': -25.274398, 'lon': 133.775136, 'population': 25688000},
        'Austria': {'lat': 47.516231, 'lon': 14.550072, 'population': 8917000},
        'Germany': {'lat': 51.165691, 'lon': 10.451526, 'population': 83200000},
        'United States': {'lat': 39.828175, 'lon': -98.579500, 'population': 331900000},
        'Japan': {'lat': 36.204824, 'lon': 138.252924, 'population': 125800000},
        'United Kingdom': {'lat': 55.378051, 'lon': -3.435973, 'population': 67500000},
        'Canada': {'lat': 56.130366, 'lon': -106.346771, 'population': 38000000},
        'France': {'lat': 46.227638, 'lon': 2.213749, 'population': 67750000},
        'China': {'lat': 35.86166, 'lon': 104.195397, 'population': 1439323776},
        'Indonesia': {'lat': -0.789275, 'lon': 113.921327, 'population': 273523615},
        'Thailand': {'lat': 15.870032, 'lon': 100.992541, 'population': 69799978},
        'Philippines': {'lat': 12.879721, 'lon': 121.774017, 'population': 109581078},
        'Vietnam': {'lat': 14.058324, 'lon': 108.277199, 'population': 97338579},
        'Malaysia': {'lat': 4.210484, 'lon': 101.975766, 'population': 32365999},
        'Russian Federation': {'lat': 61.52401, 'lon': 105.318756, 'population': 145934462},
        'Turkey': {'lat': 38.963745, 'lon': 35.243322, 'population': 84339067},
        'Ukraine': {'lat': 48.379433, 'lon': 31.16558, 'population': 43733762},
        'Kazakhstan': {'lat': 48.019573, 'lon': 66.923684, 'population': 18776707},
        'Georgia': {'lat': 42.315407, 'lon': 43.356892, 'population': 3989167},
        'Brazil': {'lat': -14.235004, 'lon': -51.92528, 'population': 212559417},
        'Mexico': {'lat': 23.634501, 'lon': -102.552784, 'population': 128932753},
        'Colombia': {'lat': 4.570868, 'lon': -74.297333, 'population': 50882891},
        'Argentina': {'lat': -38.416097, 'lon': -63.616672, 'population': 45195774},
        'Peru': {'lat': -9.189967, 'lon': -75.015152, 'population': 32971854},
        'Ecuador': {'lat': -1.831239, 'lon': -78.183406, 'population': 17643054},
        'India': {'lat': 20.593684, 'lon': 78.96288, 'population': 1380004385},
        'Pakistan': {'lat': 30.375321, 'lon': 69.345116, 'population': 220892340},
        'Bangladesh': {'lat': 23.684994, 'lon': 90.356331, 'population': 164689383},
        'Sri Lanka': {'lat': 7.873054, 'lon': 80.771797, 'population': 21413249},
        'Nepal': {'lat': 28.394857, 'lon': 84.124008, 'population': 29136808},
        'Nigeria': {'lat': 9.081999, 'lon': 8.675277, 'population': 206139589},
        'Kenya': {'lat': -0.023559, 'lon': 37.906193, 'population': 53771296},
        'South Africa': {'lat': -30.559482, 'lon': 22.937506, 'population': 59308690},
        'Ghana': {'lat': 7.946527, 'lon': -1.023194, 'population': 31072940},
        'Tanzania': {'lat': -6.369028, 'lon': 34.888822, 'population': 59734218},
        'Uganda': {'lat': 1.373333, 'lon': 32.290275, 'population': 45741007},
        'Rwanda': {'lat': -1.940278, 'lon': 29.873888, 'population': 12952218},
        'Ethiopia': {'lat': 9.145, 'lon': 40.489673, 'population': 114963588},
        'Egypt': {'lat': 26.820553, 'lon': 30.802498, 'population': 102334404},
        'Morocco': {'lat': 31.791702, 'lon': -7.09262, 'population': 36910560},
        'Jordan': {'lat': 30.585164, 'lon': 36.238414, 'population': 10203134},
        'Tunisia': {'lat': 33.886917, 'lon': 9.537499, 'population': 11818619},
        'Lebanon': {'lat': 33.854721, 'lon': 35.862285, 'population': 6825445},
        'Algeria': {'lat': 28.033886, 'lon': 1.659626, 'population': 43851044}
    }
    
    # Map countries to their regions and use REAL regional inclusion rates
    region_mapping = {}
    income_mapping = {}
    
    # Build region mapping from your data
    for region, data in regional_data.items():
        if 'income' not in region:  # This is a geographical region
            for country in data['countries']:
                region_mapping[country] = region
        else:  # This is an income group
            for country in data['countries']:
                income_mapping[country] = region
    
    # Create country dataframe using ONLY real regional data
    country_data_rows = []
    for country, coords in country_coordinates.items():
        # Find the country's region and income group from your data
        region = None
        income_group = None
        inclusion_rate = None
        
        # Search through regions
        for reg, reg_data in regional_data.items():
            if country in reg_data['countries']:
                if 'income' not in reg:
                    region = reg
                    inclusion_rate = reg_data['inclusion_rate']  # Use REAL regional rate
                else:
                    income_group = reg
        ...
        if not region:
            alt_name = country_name_mappings.get(country)
            if alt_name:
                for reg, reg_data in regional_data.items():
                    if alt_name in reg_data['countries']:
                        if 'income' not in reg:
                            region = reg
                            inclusion_rate = reg_data['inclusion_rate']
                        else:
                            income_group = reg
        
        # If still no region found, assign based on common knowledge
        if not region:
            if country in ['United States', 'United Kingdom', 'Canada', 'Australia', 'Germany', 'France', 'Japan', 'Austria']:
                region = 'High income'
                inclusion_rate = regional_data['High income']['inclusion_rate']
                income_group = 'High income'
            elif country in ['China', 'Indonesia', 'Thailand', 'Philippines', 'Vietnam', 'Malaysia']:
                region = 'East Asia & Pacific (excluding high income)'
                inclusion_rate = regional_data['East Asia & Pacific (excluding high income)']['inclusion_rate']
                income_group = 'Upper middle income'
            elif country in ['Russian Federation', 'Turkey', 'Ukraine', 'Kazakhstan', 'Georgia']:
                region = 'Europe & Central Asia (excluding high income)'
                inclusion_rate = regional_data['Europe & Central Asia (excluding high income)']['inclusion_rate']
                income_group = 'Upper middle income'
            elif country in ['Brazil', 'Mexico', 'Colombia', 'Argentina', 'Peru', 'Ecuador']:
                region = 'Latin America & Caribbean (excluding high income)'
                inclusion_rate = regional_data['Latin America & Caribbean (excluding high income)']['inclusion_rate']
                income_group = 'Upper middle income'
            elif country in ['India', 'Pakistan', 'Bangladesh', 'Sri Lanka', 'Nepal']:
                region = 'South Asia (excluding high income)'
                inclusion_rate = regional_data['South Asia (excluding high income)']['inclusion_rate']
                income_group = 'Lower middle income'
            elif country in ['Nigeria', 'Kenya', 'South Africa', 'Ghana', 'Tanzania', 'Uganda', 'Rwanda', 'Ethiopia']:
                region = 'Sub-Saharan Africa (excluding high income)'
                inclusion_rate = regional_data['Sub-Saharan Africa (excluding high income)']['inclusion_rate']
                income_group = 'Low income'
            elif country in ['Egypt', 'Morocco', 'Jordan', 'Tunisia', 'Lebanon', 'Algeria']:
                region = 'Middle East & North Africa (excluding high income)'
                inclusion_rate = regional_data['Middle East & North Africa (excluding high income)']['inclusion_rate']
                income_group = 'Lower middle income'
        
        # Add the country if we have all required data
        if region and inclusion_rate is not None:
            country_data_rows.append({
                'country': country,
                'region': region,
                'income_group': income_group if income_group else 'Unknown',
                'inclusion_rate': inclusion_rate,  # REAL rate from your data
                'lat': coords['lat'],
                'lon': coords['lon'],
                'population': coords['population']
            })
        else:
            print(f"Warning: Could not find region data for {country}")
    
    # Convert to DataFrame
    country_df = pd.DataFrame(country_data_rows)
    
    # Debug: Print column names to verify structure
    print("Country DataFrame columns:", country_df.columns.tolist())
    print("Country DataFrame shape:", country_df.shape)
    if len(country_df) > 0:
        print("Sample row:", country_df.iloc[0].to_dict())
    
    # Legacy country_data dictionary for backward compatibility (using real regional rates)
    country_data = {}
    for _, row in country_df.iterrows():
        country_data[row['country']] = {
            'region': row['region'],
            'income_group': row['income_group'],
            'inclusion_rate': row['inclusion_rate'],  # REAL rate from your data
            'lat': row['lat'],
            'lon': row['lon'],
...
<div style="text-align: center; color: #7f8c8d; margin-top: 2rem;">
    ...
    <div style="display: flex; justify-content: center; gap: 30px; margin: 1rem 0;">
        ...
        <div>🔬 <strong>Data Source:</strong> World Bank Global Findex Database 
            '<a href="https://www.worldbank.org/en/publication/global-financial-inclusion" target="_blank">Learn More</a>
            </div>         
    <p>© 2024 Global Financial Inclusion Analytics Team</p>
      </div>      
""", unsafe_allow_html=True)